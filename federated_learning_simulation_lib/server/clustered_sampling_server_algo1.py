...
from ..algorithm.clustered_sampling.algo1_utils import get_clusters_with_alg1
...
class ClusteredSamplingServerAlgo1(AggregationServer):
    ...
    def select_workers(self) -> set[int]:
        ...
        # distri_clusters is (n_sampled, n_clients) probability matrix
        # Use argmax to assign client to its primary cluster (family)
        family_dict = {}
        for client_id in range(n_clients):
            # Finding the cluster index with max probability for this client
            # col index is client_id, row index is cluster_id
            # Wait, `get_clusters_with_alg1` returns distribution for each cluster?
            # Let's check typical implementation or assume rows=clusters, cols=clients (probabilities)
            # But a client might appear in multiple?
            # Assuming standard implementation where distri_clusters[k] is prob dist over clients for k-th cluster.
            # To find which cluster a client belongs to, we look at where it has non-zero probability.
            # If strict partition, it only has non-zero in one row.
            # If soft, we take argmax over columns? No, rows are clusters.
            # We want argmax_k P(client|cluster_k)? Or P(cluster_k|client)?

            # Actually, Algo 1 partitions clients based on sorted sample sizes.
            # So a client belongs to exactly one interval (cluster) usually.
            # We can find which row `i` has non-zero value for `distri_clusters[i][client_id]`.

            assigned_cluster = -1
            max_prob = -1.0
            for cluster_id in range(n_sampled):
                prob = distri_clusters[cluster_id][client_id]
                if prob > max_prob:
                    max_prob = prob
                    assigned_cluster = cluster_id

            if assigned_cluster != -1 and max_prob > 0:
                family_dict[client_id] = assigned_cluster
        unique_clusters_found = len(set(family_dict.values()))
        logger.info("[Round %s] Clustering Algo 1: Target sampled clusters (intervals): %s, Actual populated clusters: %s",
                    self.round_index, n_sampled, unique_clusters_found)
